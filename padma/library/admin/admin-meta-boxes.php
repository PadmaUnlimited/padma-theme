...
class PadmaMetaBoxTemplate extends PadmaAdminMetaBoxAPI {
	...
	protected $inputs = array(
		'template' => array(
			...
			'type' => 'select',
			'options' => array(),
			'description' => 'Assign a shared layout to this entry.  Shared layouts can be added and modified in the Padma Visual Editor.',
...
class PadmaMetaBoxSEO extends PadmaAdminMetaBoxAPI {
	...
	protected $post_type_supports_id = 'padma-seo';
	...
	protected function modify_arguments($post = false) {
		...
		if ( get_option('page_on_front') == padma_get('post') && get_option('show_on_front') == 'page' ) {
			...
			$this->info = '<strong>Configure the SEO settings for this page (Front Page) in the Padma Search Engine Optimization settings tab in <a href="' . admin_url('admin.php?page=padma-options#tab-seo-content') . '" target="_blank">Padma &raquo; Configuration</a>.</strong>';
