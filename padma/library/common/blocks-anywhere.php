...
class PadmaBlocksAnywhere {
	...
	static function get_assets( $type ) {
		...
		if ( isset( $post->post_content ) ) {
			...
			foreach ( $this_shortcodes_ids[1] as $id )
				array_push( $shortcodes_ids, $id );

		}

		/* we push the helper ids */
		$helper_shortcodes_ids = butler_get_post_meta( 'hwr_shortcodes', 'hwr_shortcodes_helper', $post_id );

		if ( !empty( $helper_shortcodes_ids ) ) {

			foreach ( $helper_shortcodes_ids as $id )
				array_push( $shortcodes_ids, $id );
		...
		if ( !empty( $shortcodes_ids ) ) {
			...
			foreach ( $shortcodes_ids as $block_id ) {
				...
				if ( $block_id = butler_get( 'legacy_id', $block_options ) ) {
				...
				switch ( $type ) {
					...
					case 'css':
						...
						if ( file_exists( HWR_TOOLKIT_PATH . 'shortcodes/assets/css/' . $block_options['type'] . BUTLER_MIN_CSS . '.css' ) )
							$data .= butler_render( HWR_TOOLKIT_PATH . 'shortcodes/assets/css/' . $block_options['type'] . BUTLER_MIN_CSS . '.css' );
	...
	static function enqueue_assets() {
		...
		if ( self::dynamic_js() != '' ) {
			...
			$script_name = 'hwr-blocks-anywhere-dynamic-js-layout-' . PadmaLayout::get_current();

			PadmaCompiler::register_file( array(
				'name' => $script_name,
				'format' => 'js',
				'fragments' => array(
					array( 'blocksAnywhere', 'dynamic_js' )
				),
				'enqueue' => false
			) );
		...
		if ( self::dynamic_css() != '' ) {
			...
			$css_name = 'hwr-shortcodes-dynamic-css-layout-' . PadmaLayout::get_current();

			$fragments['dynamic-block-css'] = array('blocksAnywhere', 'dynamic_css');
