...
class PadmaElementsData_Upgrade34 {
	...
	public static function get_all_elements() {
		...
		$option_groups = get_option( 'padma_option_groups' );
		...
		foreach ( $design_editor_groups as $design_editor_group ) {
			...
			$group = get_option( 'padma_option_group_' . $design_editor_group );
		...
		if ( is_array( $default_data ) ) {
			$elements = padma_array_merge_recursive_simple( $default_data, $elements );
		}
	...
	public static function get_element_properties( $element_id ) {
		...
		$element = PadmaOption::get( $element_id, 'design-editor-group-' . $group, array( 'properties' => array() ) );
	...
	public static function get_special_element_properties( $element_id, $se_type, $se_meta ) {
		...
		$element = PadmaOption::get( $element_id, 'design-editor-group-' . $group, array(
	...
	public static function get_inherited_property( $element_id, $property_id, $default = null ) {
		...
		$inherit_location = PadmaElementAPI::get_inherit_location( $element_id );
	...
	public static function set_property( $element_id, $property_id, $value, $forced_group = false ) {
		...
		//Send it back to DB
		PadmaOption::set( $element_id, $element, 'design-editor-group-' . $group );
	...
	public static function delete_element( $element_id ) {
		...
		//Delete the element
		PadmaOption::delete( $element_id, 'design-editor-group-' . $group );
	...
	public static function delete_all() {
		...
		$option_groups = get_option( 'padma_option_groups', array() );
		...
		foreach ( $option_groups as $group_name => $group_name_bool_unused ) {
			...
			update_option( 'padma_option_group_' . $group_name, array() );
...
class PadmaLayoutOption_Upgrade34 {
	...
	public static function init() {
		...
		if ( padma_get( 'preview' ) && PadmaCapabilities::can_user_visually_edit() ) {

			PadmaOption::$group_suffix = '_preview';
			PadmaLayoutOption_Upgrade34::$group_suffix = '_preview';
	...
	public static function get( $layout = false, $option = null, $group_name = false, $default = null ) {
		...
		$options = get_option( 'padma_layout_options_' . str_replace( '-', '_', $layout ) . self::$group_suffix );

		if ( self::$group_suffix && ! $options ) {
			$options = get_option( 'padma_layout_options_' . str_replace( '-', '_', $layout ) );
		}
		...
		$data = padma_fix_data_type( $options[ $group_name ][ $option ] );
	...
	public static function set( $layout = false, $option = null, $value = null, $group_name = false ) {
		...
		$options = get_option( 'padma_layout_options_' . $layout );

		//Get layout options catalog
		$catalog = get_option( 'padma_layout_options_catalog' );
		...
		//Send data to DB
		update_option( 'padma_layout_options_' . $layout . self::$group_suffix, $options );

		if ( ! self::$group_suffix ) {
			update_option( 'padma_layout_options_catalog', $catalog );
		}
	...
	public static function delete( $layout, $option = null, $group_name = false ) {
		...
		if ( count( $options ) === 0 && ! self::$group_suffix ) {
			...
			$catalog = array_diff( get_option( 'padma_layout_options_catalog' ), $removal );

			delete_option( 'padma_layout_options_' . $layout );
			update_option( 'padma_layout_options_catalog', $catalog );
	...
	public static function delete_all_from_layout( $layout ) {
		...
		$catalog = array_diff( get_option( 'padma_layout_options_catalog' ), array( $layout ) );
...
class PadmaBlocksData_Upgrade34 {
	...
	protected static function schema_blocks_by_id( $use_cache = false ) {
		...
		if ( $use_cache && $cached = padma_get( 'blocks-by-id', self::$schema_cache ) ) {
			return $cached;
		}

		/* Retrieve the option from DB */
		$blocks_by_id = PadmaOption::get( 'blocks-by-id', 'blocks', array() );
	...
	protected static function schema_blocks_by_type() {
		...
		/* Retrieve the option from DB */
		$blocks_by_type = PadmaOption::get( 'blocks-by-type', 'blocks', array() );
	...
	protected static function schema_blocks_by_layout() {
		...
		/* Retrieve the option from DB */
		$blocks_by_layout = PadmaOption::get( 'blocks-by-layout', 'blocks', array() );
	...
	protected static function schema_layout_blocks( $layout_id ) {
		...
		/* Retrieve the option from DB */
		$layout_blocks = PadmaLayoutOption_Upgrade34::get( $layout_id, 'blocks', false, array() );
	...
	public static function add_block( $layout_id, $args ) {
		...
		//Update database
		PadmaLayoutOption_Upgrade34::set( $layout_id, 'blocks', $layout_blocks );

		PadmaOption::set( 'blocks-by-type', $blocks_by_type, 'blocks' );
		PadmaOption::set( 'blocks-by-id', $blocks_by_id, 'blocks' );
		PadmaOption::set( 'blocks-by-layout', $blocks_by_layout, 'blocks' );
	...
	public static function get_block( $block, $use_mirrored = false ) {
		...
		if ( is_array( $block ) ) {
			...
			if ( ! isset( $block['id'] ) && ! padma_get( 'new', $block, false ) ) {
				return null;
			}
			...
			$layout_blocks = self::get_blocks_by_layout( padma_get( 'layout', $blocks_by_id[ $block ] ) );
	...
	public static function get_blocks_by_layout( $layout_id, $include_design_editor_instances = false ) {
		...
		foreach ( $layout_blocks as $block_id => $block ) {
			...
			if ( $include_design_editor_instances ) {
				...
				$block_element = PadmaElementAPI::get_element( 'block-' . $layout_blocks[ $block_id ]['type'] );
				...
				foreach ( padma_get( 'children', $block_element, array() ) as $block_element_sub_element ) {
	...
	public static function get_blocks_by_wrapper( $layout_id, $wrapper_id ) {
		...
		foreach ( $layout_blocks as $block_id => $block ) {
			...
			if ( padma_get( 'wrapper', $block, PadmaWrappers::$default_wrapper_id ) === $wrapper_id ) {
	...
	public static function get_block_name( $block ) {
		...
		$default_name = PadmaBlocks::block_type_nice( $block['type'] ) . ' #' . $block['id'];

		return padma_get( 'alias', $block['settings'], $default_name );
	...
	public static function get_block_width( $block ) {
		...
		$wrapper = PadmaWrappers::get_wrapper( padma_get( 'wrapper', $block, 'wrapper-default' ) );

		return ( $block_grid_width * ( padma_get( 'column-width', $wrapper ) + padma_get( 'gutter-width', $wrapper ) ) ) - padma_get( 'gutter-width', $wrapper );
	...
	public static function is_block_mirrored( $block, $return_block_id = false ) {
		...
		if ( $block && $mirrored_block_id = padma_get( 'mirror-block', $block['settings'] ) ) {
			...
			if ( padma_get( 'type', $mirrored_block ) != padma_get( 'type', $block ) ) {
				return false;
			}

			/* Make sure that the mirrored block isn't mirroring another block */
			$possible_mirror_of_mirror = padma_get( 'mirror-block', $mirrored_block['settings'] );

			if ( $possible_mirror_of_mirror && $mirror_of_mirror_block = self::get_block( $possible_mirror_of_mirror ) ) {
				if ( padma_get( 'type', $mirror_of_mirror_block ) == padma_get( 'type', $mirrored_block ) ) {
					return false;
				}
			}
