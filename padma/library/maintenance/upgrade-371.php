...
function padma_do_upgrade_371() {
	...
	foreach ( $templates as $template_id => $template ) {
		...
		$template_design_settings = get_option( 'padma_|template=' . $template_id . '|_option_group_design', array() );

		if ( !empty($template_design_settings) ) {

			$template_design_settings = padma_preg_replace_json( "/-layout-[\w-]*/", '', $template_design_settings );
			update_option( 'padma_|template=' . $template_id . '|_option_group_design', $template_design_settings );
